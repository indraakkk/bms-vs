...
- **[E][Resolved 2026-07-17] occupancy fill hues** — PDF says "e.g. green/yellow/red" (soft); mock used gray/green/red. Owner decided for the PDF: `--occ-*` swapped to green/yellow(amber)/red in both themes, with new paired `--occ-*-ink` tokens replacing the hardcoded white zone-label text (white fails on yellow). The gray-low vs. stale-gray thinning noted below is gone as a side effect.
