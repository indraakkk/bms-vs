...
import { IconExport, IconImport, IconPlus } from "@/components/icons";
  ...
  return (
    <header className="flex items-center gap-3.5 border-b bg-card px-[22px] pt-[15px] pb-[13px]">
      ...
      <button
        ...
        className={cn(
          "flex items-center gap-[7px] rounded-[9px] border px-[13px] py-2 font-semibold text-[12.5px] transition-colors",
          paletteOpen
...
function HeaderButton({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex items-center gap-[7px] rounded-[9px] border bg-secondary px-[13px] py-2 font-semibold text-[12.5px] text-muted-foreground transition-colors hover:border-border-strong hover:text-foreground"
    >
      {children}
    </button>
  );
