...
export function Sidebar() {
  ...
  // Offcanvas collapse (dashboard-01 style): the outer shell animates
  // width while the inner column keeps its fixed 238px, so the content
  // doesn't reflow mid-transition — it slides out of the clip instead.
  return (
    <div
      className={cn(
        "shrink-0 overflow-hidden transition-[width] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] print:hidden",
        open ? "w-[238px]" : "w-0",
      )}
    >
      <aside className="flex h-full w-[238px] flex-col gap-1 border-r bg-card px-3.5 py-[18px]">
...
/** Avatar + name + detail — shared between the trigger and the menu label. */
function UserIdentity() {
  return (
    <>
      <div className="flex size-[30px] shrink-0 items-center justify-center rounded-lg bg-surface-3 font-bold text-[12px] text-muted-foreground">
        FO
      </div>
      <div className="min-w-0 flex-1 text-left leading-[1.2]">
        <div className="truncate font-semibold text-[12.5px] text-foreground">Facilities Ops</div>
...
/** shadcn dashboard-01's nav-user pattern, restyled to this design:
 *  the sidebar-footer identity row is the trigger; session actions
 *  (Lock console / logout) live in its menu. */
function NavUser() {
    ...
    <div className="border-t pt-2">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            className="flex w-full items-center gap-2.5 rounded-[10px] px-2 py-[7px] transition-[background-color,scale] hover:bg-accent/50 active:scale-[0.98] data-[state=open]:bg-accent"
          >
            <UserIdentity />
            <IconChevronUpDown size={14} className="shrink-0 text-fg-subtle" />
          </button>
        </DropdownMenuTrigger>
        <DropdownMenuContent side="right" align="end" sideOffset={10} className="w-[212px]">
